...
ComandReceiver::ComandReceiver(ros::NodeHandle *n,  MoveBaseClient &acc)
{
//    subdir = n->subscribe("odom", 1, &ComandReceiver::Setdir, this);
    ...
    pubdest = n->advertise<geometry_msgs::PoseStamped>("move_base_simple/goal", 1);
    ac = &acc;
...
void ComandReceiver::Setdir(geometry_msgs::TransformStamped msg)
{
    ...
    tf2::Quaternion quat(msg.transform.rotation.x, msg.transform.rotation.y, msg.transform.rotation.z, msg.transform.rotation.w);
    ...
    Tw.angular.y = 0;
    Tw.angular.z = 0;
    pos.setX(msg.transform.translation.x);
    pos.setY(msg.transform.translation.y);
    pos.setZ(0);
    dir = rotated_vector;
...
//        pause = true;
    ...
    angle = (pos - pPos).angle(pDir);
    tf2::Vector3 newdest = pPos + pDir * (((pos - pPos).length()) * cos(angle)) + pDir * 10.f;
    if ((dest - pPos).length() > (newdest - pPos).length())
        dest = newdest;
    angle = pDir.angle(vecto);
    if (tf2::tf2Cross(pDir, vecto).z() > 0)
    {
        angle *= -1;
    }
    quat.setRPY(0, 0, angle);
    geometry_msgs::PoseStamped pointmsg;
    pointmsg.header.frame_id = "map";
    pointmsg.header.stamp = ros::Time::now();
    pointmsg.pose.orientation = tf2::toMsg(quat);
    pointmsg.pose.position.x = dest.x();
    pointmsg.pose.position.y = dest.y();
    pointmsg.pose.position.z = dest.z();
    pubdest.publish(pointmsg);
    ...
    goal.target_pose.pose.position = pointmsg.pose.position;
    goal.target_pose.pose.orientation = pointmsg.pose.orientation;
...
int main(int ac, char** av)
{
   ...
   while(n.ok())
   {
       ...
       comand->Setdir(transformStamped);
       r.sleep();
       ros::spinOnce();
   }
    return 0;
